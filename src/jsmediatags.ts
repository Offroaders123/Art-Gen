...
function toMediaTags({ title, artist, album, artwork }: AllTags): MediaTags {
  if (title === undefined) throw new Error("Could not load title from song");
  if (artist === undefined) throw new Error("Could not load artist from song");
  if (album === undefined) throw new Error("Could not load album from song");
  if (artwork === undefined) throw new Error("Could not load artwork from song");
